<template>
	<div class="diy_details_box diy_div_tourism_type">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "tourism_type_id",
					source_id: this.obj["tourism_type_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/tourism_type/set?tourism_type_id=' + _this.obj["tourism_type_id"], {
								praise_len
							}, (res) => {
								if(res.result){
