<template>
	<div class="diy_details_box diy_div_mode_of_transportation">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "mode_of_transportation_id",
					source_id: this.obj["mode_of_transportation_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/mode_of_transportation/set?mode_of_transportation_id=' + _this.obj["mode_of_transportation_id"], {
								praise_len
							}, (res) => {
								if(res.result){
